#!/bin/bash
echo '-----------------------------------------------------------------------------------------------'
echo '---- UPDATING THE SYSTEM FOR AWS EC2 MULTIUSER STUDENT DEVELOPER UBUNTU 22.04 XRDP GUI SERVER'
echo '---- This is the second part of the setup script and deals with adding an xrdp Mate GUI.'
echo '---- chmod 700 d2.sh and run as root.'
echo '---- version 20160821 updated 20240229'
echo '-----------------------------------------------------------------------------------------------'
...
useradd -m macy -c 'Macy' -s '/bin/bash'
echo macy:mucis | sudo chpasswd
usermod -aG sudo macy
echo '--'
